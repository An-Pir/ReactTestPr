  ...
  return (
    ...
    >
      <form
        ...
        className=' flex flex-col items-center gap-6 shadow-lg p-15  bg-white rounded-md absolute'
      >
        <AuthFormTitle title='Регистрация' />

        {error && (
          <div className='text-lg text-center font-medium text-red-500 underline'>
            {error}
          </div>
        )}
        {success && (
          <div className='text-lg font-medium text-green-500 underline'>
            {success}
          </div>
        )}
        ...
        <label className='w-[290px] flex text-[12px] text-orange'>
          ...
          <span
            className={`flex items-center justify-center w-5 h-5 border-2 ${
              checked ? 'border-orange' : 'border-light-blue'
            } rounded-md mr-2`}
          >
        ...
        <Button
          name={loading ? 'Загрузка...' : 'Зарегистрироваться'}
